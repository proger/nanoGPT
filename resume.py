...
wandb_entity = 'stud76'
wandb_project = 'uk4b'
wandb_run_name = 'large' # 'run' + str(time.time())
# data
dataset = 'uk4b'
...
max_iters = 30<<20 # total number of training iterations
weight_decay = 1e-2
betas = (0.9, 0.95)
# learning rate decay settings
decay_lr = True # whether to decay the learning rate
warmup_iters = (1 << 20) + 100*grad_acc_steps # how many steps to warm up for
lr_decay_iters = max_iters - warmup_iters # how many steps to decay the learning rate for
min_lr = 2e-4 # minimum learning rate
# DDP settings
...
if init_from == 'scratch':
    ...
    ckpt_path = os.path.join(out_dir, 'last-2.pt')
...
print('run_config = ', run_config.keys(), flush=True)
# logging
...
while True:
    ...
    if iter_num % eval_interval == 0 and gpu_id == 0:
        ...
        if losses['val'] < best_val_loss:
            ...
            if iter_num > 0:
                torch.save(checkpoint, os.path.join(out_dir, 'ckpt-2.pt'))
        else:
            torch.save(checkpoint, os.path.join(out_dir, 'last-2.pt'))
